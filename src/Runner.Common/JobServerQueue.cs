...
namespace GitHub.Runner.Common
{
    ...
    public interface IJobServerQueue : IRunnerService, IThrottlingReporter
    {
        ...
        Task ShutdownAsync();
        void Start(Pipelines.AgentJobRequestMessage jobRequest, bool enableActionsUpload);
    ...
    public sealed class JobServerQueue : RunnerService, IJobServerQueue
    {
        ...
        private bool _enableActionsUpload = true;
        ...
        public void Start(Pipelines.AgentJobRequestMessage jobRequest, bool enableActionsUpload)
        {
            Trace.Entering();
            _enableActionsUpload = enableActionsUpload;
            ...
            _websocketFeedServer.InitializeWebsocketClient(serviceEndPoint, fallback: enableActionsUpload ? _jobServer : null);
            ...
            while (!_jobCompletionSource.Task.IsCompleted || runOnce)
            {
                ...
                if (pendingUpdates.Count > 0)
                {
                    foreach (var update in pendingUpdates)
                    {
                        ...
                        {
                            if (!_allTimelines.Contains(detailTimeline.Details.Id) && _enableActionsUpload)
                            {
                                try
                                {
