...
namespace GitHub.Runner.Common
{
    ...
    {
        void InitializeWebsocketClient(ServiceEndpoint serviceEndpoint, IJobServer fallback);
    ...
    public sealed class WebsocketFeedServer : RunnerService, IWebsocketFeedServer
    {
        ...
        private IJobServer _fallback;

        public void InitializeWebsocketClient(ServiceEndpoint serviceEndpoint, IJobServer fallback)
        {
        ...
        public async Task AppendTimelineRecordFeedAsync(Guid scopeIdentifier, string hubName, Guid planId, Guid timelineId, Guid timelineRecordId, Guid stepId, IList<string> lines, long? startLine, CancellationToken cancellationToken)
        {
            ...
            if (!pushedLinesViaWebsocket && !cancellationToken.IsCancellationRequested && _fallback != null)
            {
                if (startLine.HasValue)
                {
                    await _fallback.AppendTimelineRecordFeedAsync(scopeIdentifier, hubName, planId, timelineId, timelineRecordId, stepId, lines, startLine.Value, cancellationToken: cancellationToken);
                }
                else
                {
                    await _fallback.AppendTimelineRecordFeedAsync(scopeIdentifier, hubName, planId, timelineId, timelineRecordId, stepId, lines, null, cancellationToken: cancellationToken);
                }
            }
        }
